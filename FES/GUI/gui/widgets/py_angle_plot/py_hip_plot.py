...
class PyHipPlot(pg.PlotWidget):
    def __init__(
        ...
        self._left_hip = deque(maxlen=max_points)
        self._right_hip = deque(maxlen=max_points)
        self.left_hip_enabled = False
        self.right_hip_enabled = False
        ...
        self.left_hip_curve = self.plot(pen=pen_left)
        pen_right = pg.mkPen(color=line_color_right, width=3)
        self.right_hip_curve = self.plot(pen=pen_right)
    ...
    def show_left_hip_angle(self, do_show: bool = False):
        self.left_hip_enabled = do_show

    def show_right_hip_angle(self, do_show: bool = False):
        self.right_hip_enabled = do_show
    ...
    def get_scale_factors(self) -> tuple[float, float]:
        """
        Returns the current scale factors for left and right hip angles.
        """
    ...
    def reset_plot(self):
        self.ptr = 0
        self._time.clear()
        self._left_hip.clear()
        self._right_hip.clear()
    ...
    def update_plot(self):
        try:
            ...
            left_hip_angle, right_hip_angle = self.calibrator.get_latest_hip_data()

            self.ptr += 1
            self._time.append(self.ptr)

            lka = np.asarray(left_hip_angle)
            if self.left_hip_enabled and lka.size > 0:
                self._left_hip.append(float(lka.flat[0]))
            else:
                self._left_hip.append(self._left_hip[-1] if self._left_hip else 0.0)

            rka = np.asarray(right_hip_angle)
            if self.right_hip_enabled and rka.size > 0:
                self._right_hip.append(float(rka.flat[0]))
            else:
                self._right_hip.append(self._right_hip[-1] if self._right_hip else 0.0)

            t = np.array(self._time, dtype=float)
            lk = np.array(self._left_hip, dtype=float) * self.scale_factor_left
            rk = np.array(self._right_hip, dtype=float) * self.scale_factor_right

            self.left_hip_curve.setData(t, lk)
            self.right_hip_curve.setData(t, rk)
            ...
            print(f"[HipPlot] update_plot error: {exc}")
