"""Phase-to-muscle mapping and stimulation-channel control.

Maps each ``Phase`` to the muscle targets per side (``MUSCULAR_GROUP_SELECTION``
and the distal-free ``_2`` variant: TA/GA/VM/BF/GM/RF + electrode groups) and
provides the functions that open/update the stimulation channels for the IMU,
FSR and fused pipelines, run the fixed FES step, apply the closed-loop offset
and ramp the current over 10 s.
"""
