...
idx_start = np.argmin(np.abs(t_rel - 124))
idx_end = np.argmin(np.abs(t_rel - 125.5))

print(f"Cycle 124-125s SAA min: {np.min(saa_knee[idx_start:idx_end]):.2f}")
print(f"Cycle 124-125s SAA max: {np.min(saa_knee[idx_start:idx_end]):.2f}") # wait, max!
print(f"Cycle 124-125s SAA MAX: {np.max(saa_knee[idx_start:idx_end]):.2f}")
